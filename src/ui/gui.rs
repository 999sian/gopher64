...
fn open_detail(handle: &AppWindow, path: &str) {
    ...
    {
        // Save-state availability: the whole-ROM read + hash stays off the UI thread.
        let weak = handle.as_weak();
        let p = path.to_string();
        tokio::spawn(async move {
            let pc = p.clone();
            if let Ok(slots) = tokio::task::spawn_blocking(move || savestate_slots(&pc)).await {
                let _ = weak.upgrade_in_event_loop(move |h| {
                    ...
                    if h.get_library_detail_open()
                        && h.get_games()
                            ...
                            .unwrap_or(false)
                    {
                        h.set_library_detail_slots(slint::ModelRc::from(std::rc::Rc::new(
    ...
    handle.set_library_detail_open(true);
    {
        ...
        if title.is_some() {
            let weak = handle.as_weak();
            tokio::spawn(async move {
                ui::boxart::resolve_media(&p).await;
                let _ = weak.upgrade_in_event_loop(move |h| {
                    if !h.get_library_detail_open() {
                        return;
                    }
...
async fn fetch_all_art(weak: slint::Weak<AppWindow>) {
    let Some(no_intro_map) = NO_INTRO_MAP.get().cloned() else {
        return;
    };
    ...
    for meta in &games {
        ...
        let Ok(Some(rom)) = tokio::task::spawn_blocking(move || {
            crate::device::get_rom_contents(&std::path::PathBuf::from(&path))
        })
        .await
        else {
            continue;
        };
        // No-Intro name + homebrew flag from the shared DB — the same lookup
        // ui::gui::get_nointro_name does, computed once here so we also learn
        // whether the hash was absent (i.e. homebrew/hack/bad dump).
        let hash = crate::device::cart::rom::calculate_hash(&rom).to_lowercase();
        let (name, homebrew) = match no_intro_map.lock().await.get(&hash) {
            Some(n) => (n.clone(), false),
            None => (ui::storage::get_game_name(&rom), true),
        };
        ui::boxart::set_homebrew(&meta.path, homebrew);
        let arted = ui::boxart::resolve_and_cache(&meta.path, &name).await;
...
fn setup_menu_gamepad(app: &AppWindow) {
    ...
    timer.start(
        ...
        move || {
            ...
            // While a game runs, drain+discard queued gamepad presses so they don't
            // replay as phantom menu actions (relaunch) once it exits.
            if handle.get_game_running() {
                unsafe {
...
fn local_game_window(app: &AppWindow, config: &ui::config::Config) {
    ...
    *FAVORITES.lock() = config.ui.favorites.iter().cloned().collect();
    *LIBRARY.lock() = recent_meta(config);
    ...
    app.on_toggle_favorite(move |path| {
        ...
        config.ui.favorites = FAVORITES.lock().iter().cloned().collect();
        drop(config);
        if let Some(handle) = weak.upgrade() {
            apply_library(&handle);
        }
...
// Shared No-Intro DB handle, set once from app_window() below so the library's
// background art fetch can resolve names without threading the Arc through the
// whole run path (run_rom -> update_recent_roms). OnceLock, not LazyLock: the
// value is app_window's runtime parameter, not a declaration-time initializer.
